...
describe("filterComponentsForTarget", () => {
  ...
  test("opencode target returns all supported components on linux", () => {
    const result = filterComponentsForTarget("opencode", linuxPlatform)
    expect(result.sort()).toEqual(["codebase-memory", "context-mode", "plugin", "rtk", "sdd-integration", "sounds", "theme", "tmux", "tui-plugins"])
    expect(result).not.toContain("otel" as never)
    expect(result).not.toContain("otel-collector" as never)
  })
